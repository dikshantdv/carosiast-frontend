...
const CarItemImage = styled((props) => <Box {...props} />)(({ theme }) => ({
  width: 288,
  height: 192,
    ...
    <CarItem direction={props?.direction}>
      ...
      <CarItemSummary>
        ...
        <Carousel
          ...
          touch={true}
          prevIcon={
            <CustomFab size="small" sx={{ left: -10 }}>
              <KeyboardArrowLeftRoundedIcon />
            </CustomFab>
          }
          nextIcon={
            <CustomFab size="small" sx={{ right: -10 }}>
        ...
        >
          {props?.data.variants.map((variant) => (
            <Carousel.Item
              key={`variant-carousel-${variant?._id}`}
              className="py-1 px-4"
            >
              <Tooltip title={variant?.name} arrow>
                <NavLink
                  to="/car/detail"
                  state={{ _id: props?.data?.carId, variant: variant?._id }}
                >
                  <Button
                    color="error"
                    fontSize={18}
      ...
      {loading ? (
        ...
        >
          ...
          <Box
            className="d-grid"
            sx={{
              gridTemplateColumns: "repeat(auto-fill,minmax(288px,288px))",
              gap: 5,
              width: "100%",
            }}
          >
            {data.map((car) => (
              <CarSummaryCard
                ...
                data={{
                  ...
                  price_range: `Rs. ${priceAbbr(car?.minPrice)} - ${priceAbbr(
                    car?.maxPrice
                  )}`,
                  rating: 4.5,
                  carId: car?._id,
