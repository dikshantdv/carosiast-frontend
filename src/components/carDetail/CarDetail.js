...
import { Box, Rating, Stack, Typography, Accordion, AccordionSummary, AccordionDetails, Paper, Table, TableBody, TableCell, TableRow, Dialog, DialogTitle, DialogContent, ListItemButton, List, } from '@mui/material';
...
// STYLED COMPONENTS
const CarDetailImage = styled((props) => (
    <Box {...props} />
))(({ theme }) => ({
    width: '100%',
    padding: 0,
...
function CarDetail() {
    ...
    const selectedVariantData = useSelector(state => state.detail.selectedVariant)
    ...
    // Select variant
    const selectedVariantId = useSelector(state => state.detail.selectedVariant._id)
        ...
        <>
            {(loading) ? <LoadingScreen />
                ...
                <Box className='mx-auto' pb={3} sx={{ width: '85vw' }} >
                        ...
                        <CarDetailImage>
                            ...
                            >
                                {detailData.images && detailData.images.map((image, index) =>
                                    <img key={`carousel-image-${index}`} className='car-image' src={image} alt="" />
                                )}
                            </Carousel>
                        </CarDetailImage>
                        <Stack spacing={1} py={2}>
                            <Typography className='text-capitalize' fontSize={36} fontWeight={600} sx={{ color: 'var(--primary-color) !important' }}>{detailData.company} {detailData.name}</Typography>
                            <Typography className='text-capitalize' fontSize={32} fontWeight={600} sx={{ color: 'var(--secondary-color) !important' }}>{selectedVariantData.name}</Typography>
                            <div>
                                <Typography fontSize={28} fontWeight={500}>Rs. {priceAbbr(selectedVariantData.price)}*</Typography>
                            ...
                            <Typography fontSize={24} fontWeight={500}>Colours</Typography>
                            <Stack direction='row' spacing={1.5}>
                                {['black', 'grey', 'white', '#2c387e', '#b52a2a', 'orange'].map(colour => (<ColourSelector key={colour} colour={colour} />))}
                            </Stack>
                            <Stack direction='row' spacing={3} pt={2}>
                                ...
                                <OptionBox onClick={handleEmiBoxOpen}>
                                    <Box className='option-detail'>
                                        <Typography className='text-capitalize'>{detailData.name} EMI</Typography>
                                        <Typography>Rs.{emiCalculate(selectedVariantData.price * 0.9, 9, 5)} per month</Typography>
                                    </Box>
                                    <Stack justifyContent='center' className="align-middle">
                                        <ChevronRightRoundedIcon fontSize="large" />
                                    </Stack>
                    ...
                    <Box pl={1}>
                        <Typography variant="h4" fontWeight={600} sx={{ color: 'var(--secondary-color) !important' }} className='text-left'>Key Specifications</Typography>
                    </Box>
                    <Box>
                        <KeySpecificationBox mt={3} sx={{ pointerEvents: 'none' }}>
                            {keySpecification.map(item => <KeySpecificationItem
                                key={`specification-${item.name}`}
                            >
                                <Image fluid rounded src={item.image} alt="" width={64} />
                                <Typography fontSize={18} fontWeight={500} className='text-center'>{item.name}</Typography>
                                <Typography fontSize={16} fontWeight={500} className='text-center' color="#4D4C4C">{item.value}</Typography>
                            </KeySpecificationItem>)}
                        </KeySpecificationBox>
                    </Box>
                    <Box pl={1} mt={3}>
                        <Typography variant="h4" fontWeight={600} sx={{ color: 'var(--secondary-color) !important' }} className='text-left'>All Specifications</Typography>
                    </Box>
                    <Box mt={3}>
                        <Paper elevation={5}>
                            {carData.map(row =>
                                <Accordion key={`specification-${row.title}`} disableGutters>
                                    ...
                                    <AccordionDetails>
                                        <Table sx={{ padding: 50, [`& .MuiTableRow-root:last-child td`]: { borderBottomWidth: "0px !important", }, }}>
                                            <TableBody>
                                                {row.data.map(spec =>
                    ...
                    {/* Variant Box */}
                    <Dialog open={openVariantBox} sx={{ '& .MuiDialog-paper': { borderRadius: '15px', bgcolor: '#EFEFEF' } }} onClose={handleVariantBoxClose}>
                        <DialogTitle>
                            <Typography fontWeight={600} fontSize={32}>Variants</Typography>
                        </DialogTitle>
                        <DialogContent sx={{ padding: '0 !important' }}>
                            {allVariantsData && allVariantsData.map(variant =>
                                <List component="nav" disablePadding key={variant._id}
                                    sx={{ borderTop: '1px solid #4D4C4C', minWidth: 420 }}>
                                    <ListItemButton
                                        selected={selectedVariantIndex === variant._id}
                                        onClick={(event) => handleVariantClick(event, variant._id)}
                                    >
                                        <Stack
                                            direction='row'
                                            spacing={10}
                                        ...
                                        >
                                            <Stack direction='column' sx={{ minWidth: 164 }}>
                                                <Typography fontWeight={500} fontSize={20} sx={{ flex: '1 1 100%', color: 'var(--primary-color) !important' }}>{variant.name}</Typography>
                                                <Stack direction="row" spacing={3} justifyContent="space-between">
                                                    <Typography fontWeight={600} fontSize={14} sx={{ color: '#4D4C4C !important', textTransform: 'capitalize' }}>{variant.fuel}</Typography>
                                                    <Typography fontWeight={600} fontSize={14} sx={{ color: '#4D4C4C !important', textTransform: 'capitalize' }}>{variant.transmission}</Typography>
                                                </Stack>
                                            </Stack>
                                            <Stack direction='column' spacing={2}>
                                                <Typography fontWeight={500} fontSize={20} sx={{ color: '#4D4C4C !important' }}>₹ {priceAbbr(variant.price)}</Typography>
                                            </Stack>
                                        </Stack>
                                    </ListItemButton>
                                </List>)}
                    ...
                    {/* Emi calculator */}
                    <EmiCalculator open={openEmiBox} price={selectedVariantData.price} emiCalculate={emiCalculate} handleEmiBoxClose={handleEmiBoxClose} />
                </Box>
            }
        </>
    )
...
const carData = [
    {
        ...
        data: [
            { key: "Engine", value: "1197 cc, 4 Cylinders Inline, 4 Valves/Cylinder, DOHC", },
    ...
    },
    {
        ...
        data: [
            ...
            { key: "Rear Suspension", value: "Coupled Torsion Beam Axle with Coil Spring", },
    ...
    },
    {
        ...
        data: [
            { key: "Overspeed Warning", value: "1 beep over 80kmph, Continuous beeps over 120kmph", },
