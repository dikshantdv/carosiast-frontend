...
export function CarSummaryCard(props) {
    return (
        <CarItem direction={props.direction}>
            ...
            <CarItemSummary>
                <Typography sx={{ fontSize: "1.25rem", fontWeight: 600, textTransform: 'capitalize' }}>{props.data.name}</Typography>
                <Typography sx={{ fontSize: "1.25rem", fontWeight: 500, }}>{props.data.price_range}</Typography>
...
export default function CarList(props) {
    ...
    useEffect(() => {
        axios.get(props.url)
            .then(response => { setCarsData(response.data.cars) })
                                ...
                                >
                                    <CarSummaryCard
                                        ...
                                        data={{
                                            image: item.images[0],
                                            name: `${item.company} ${item.name}`,
                                            price_range: `Rs. ${priceAbbr(item.minPrice)} - ${priceAbbr(item.maxPrice)}`,
                                            rating: 4.5,
                                        }}
                                    />
                                </NavLink>
